...
public abstract class CustomEvent {
    ...
    public string GetString(string key) => BeatmapEvent.GetFirstEventDataAsString(key);
    ...
    public string[] GetMatchingTypes() {
        var typeSegments = $"{Plugin.NAME}.{Type}".ToLowerInvariant().Split('.');
    ...
    public void FlagForProcessing() {
        foreach(var type in GetMatchingTypes()) {
            BeatmapEvent.AddEventData($"__MODS__.{type}", Plugin.GUID);
        }
    }
    ...
    public bool ShouldProcess() {
        ...
        foreach(var type in types) {
            ...
            if(!string.IsNullOrEmpty(processor)) {
                return processor == Plugin.GUID;
            }
        }
        return true; // no mod has claimed it—mark it for ourselves
    }

    public virtual bool IsValid() {
        return GetMatchingTypes().Length > 0 && ShouldProcess();
    }
